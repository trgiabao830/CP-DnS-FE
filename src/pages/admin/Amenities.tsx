...
import type { HomestayAmenity } from "../../types/admin/amenity";
...
const Amenities: React.FC = () => {
  ...
  const [editingItem, setEditingItem] = useState<HomestayAmenity | null>(null);

  const [deleteId, setDeleteId] = useState<number | null>(null);
  const [statusItem, setStatusItem] = useState<HomestayAmenity | null>(null);

  const [successMsg, setSuccessMsg] = useState("");
  const [errorMsg, setErrorMsg] = useState("");
  const [isPermissionDenied, setIsPermissionDenied] = useState(false);

  const fetchAmenitiesRef = useRef(fetchAmenities);
  ...
  useEffect(() => {
    ...
    console.log("Connecting to SSE (Amenities)...");
    const eventSource = new EventSource(`${API_BASE_URL}/sse/subscribe`);

    eventSource.onopen = () => {
      console.log("SSE Connected (Amenities)!");
    };

    eventSource.addEventListener("AMENITY_UPDATE", (event) => {
      if (isActive) {
        console.log("Received SSE Event: AMENITY_UPDATE", event.data);
        if (fetchAmenitiesRef.current) {
    ...
    return () => {
      console.log("Closing SSE connection (Amenities)...");
      isActive = false;
      eventSource.close();
    };
  }, []);
  ...
  const handleFormSubmit = async (data: any) => {
    if (editingItem) {
      await updateAmenity(editingItem.amenityId, data);
      setSuccessMsg("Cập nhật tiện ích thành công!");
    } else {
      await createAmenity(data);
      setSuccessMsg("Thêm tiện ích mới thành công!");
    }
  ...
  const handleStatusChangeConfirm = async () => {
    ...
    try {
      ...
      await updateStatus(statusItem.amenityId, newStatus);
      setSuccessMsg(
        `Đã chuyển trạng thái sang ${newStatus === "ACTIVE" ? "Hiển thị" : "Ẩn"}`,
      );
      setStatusItem(null);
    } catch (err: any) {
      handleError(err);
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div className="min-w-fit">
          ...
          <p className="text-sm text-gray-500">
            Danh sách các tiện nghi có tại Homestay
          </p>
        </div>
                      ...
                      >
                        ...
                        <td className="px-6 py-4 text-center">
                          <div className="flex justify-center gap-2">
                            ...
                            <button
                              onClick={() => setDeleteId(item.amenityId)}
      ...
      <DeleteConfirmModal
        ...
        title="Xóa tiện ích?"
        message="Hành động này sẽ xóa tiện ích khỏi hệ thống. Bạn có chắc chắn không?"
      />
        ...
        >
          <div className="p-6 text-center">
            ...
            <h3 className="mb-2 text-lg font-bold text-gray-800">
              {statusItem.status === "ACTIVE"
                ? "Ngưng hoạt động tiện ích này?"
                : "Kích hoạt tiện ích này?"}
            </h3>
            <p className="mb-6 text-sm text-gray-500">
              Bạn có chắc muốn{" "}
              {statusItem.status === "ACTIVE" ? "ngưng hoạt động" : "kích hoạt"}{" "}
              tiện ích <strong>{statusItem.name}</strong>?
            </p>
