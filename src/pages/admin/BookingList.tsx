...
import type { AdminBookingSummary, BookingStatus } from "../../types/admin/booking";
...
const BookingList: React.FC = () => {
  ...
  const [fromDate, setFromDate] = useState<Date | null>(() => getInitialDate("fromDate", new Date()));
  const [toDate, setToDate] = useState<Date | null>(() => getInitialDate("toDate", addDays(new Date(), 7)));
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const res = await restaurantService.getBookings({
        page: meta.page,
        size: meta.size,
    ...
    <div className="space-y-6">
      <div className="flex flex-col gap-4">
        <div className="flex flex-wrap items-center justify-between gap-4">
          <div>
            ...
            <p className="text-sm text-gray-500">
              Theo dõi và xử lý các đơn đặt bàn từ khách hàng
            </p>
          </div>
        ...
        <div className="grid grid-cols-1 gap-3 rounded-xl border border-gray-200 bg-white p-4 shadow-sm md:grid-cols-12">
          ...
          {/* Date Range: Sử dụng React Datepicker */}
          <div className="relative z-10 md:col-span-2">
            {" "}
            {/* z-10 để dropdown lịch đè lên table */}
                  ...
                  {bookings.length === 0 ? (
                    ...
                    bookings.map((item) => (
                      ...
                      >
                        ...
                        <td className="px-6 py-4">
                          <div className="font-medium">{item.tableNumber}</div>
                          <div className="text-xs text-gray-500">
                        ...
                        <td className="px-6 py-4 text-center">
                          <Link
                            to={`/admin/booking/${item.bookingId}`}
